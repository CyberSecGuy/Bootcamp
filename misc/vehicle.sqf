...
_unitside = if (count _this > 7) then {_this select 7} else {side _unit;};
...
{    
    ...
    {    
        ...
        deleteVehicle _unit;
        sleep 2;
		_unit = createVehicle [_type,_position,[],_dir,"none"];
		_unit setPosASL _position;
		_unit setDir _dir;

		//_unit = createVehicle [_type,_position, [],_dir,"NONE"];
		if(getNumber(configFile >> "CfgVehicles" >> typeof _unit >> "isUav") == 1) then
		{
